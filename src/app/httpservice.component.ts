...
@Component({
	...
	class="form-control">
	<input (keyup.enter)="createPost(id)" #id type=number" class="form-control">

    <ul class="list-group">
     <li *ngFor="let post of posts" class="list-group-item">
     
     <button (click)="updatePost(post)" class="btn btn-info btn-sm">
     Update </button>
...
export class HTTPServiceComponent{
    ...
    createPost(title: HTMLInputElement){
        ...
        JSON.stringify(post)).subscribe(response => {
            post ['id']= response.json().id;
            this.posts.splice(0,0,post)
        });
	}
	createPost(title: HTMLInputElement){
        let post= { id: id.value };
        id.value = ' ';
   ...
   updatePost(post){
       this.http.patch('https://jsonplaceholder.typicode.com/posts'+'/'+post.id,
       JSON.stringify({ isUpdate : true  })).subscribe(response => {
               console.log(response.json());
           });
   }
